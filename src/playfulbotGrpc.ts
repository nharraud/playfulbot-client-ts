...
import { FollowGameResponse } from './grpc/proto/types/playfulbot/v0/FollowGameResponse';
...
export class PlayfulBotGrpc<GS extends GameState> {
  ...
  constructor(endpoint: string, token: string, botAI: BotAI<GS>) {
  ...
  async playGame(gameID: string, client: ClientInterfaces.playfulbot.v0.PlayfulBotClient, authMetadata: grpc.Metadata): Promise<void> {
